"""
Utility functions for the Telegram Confession Bot
...
async def handle_admin_search_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle admin search input for user ID or name"""
    user_id = update.message.from_user.id
    ...
    search_type = context.user_data.get('admin_search_type')
    search_input = update.message.text.strip()
    
    if not search_input:
        await update.message.reply_text("❗ Please provide a valid search term.")
        return
    
    try:
        from db import search_user_by_id, search_users_by_name
        
        if search_type == 'search_by_id':
            # Try to parse as integer ID
            try:
                search_id = int(search_input)
                user_data = search_user_by_id(search_id)
                
                if user_data:
                    await display_user_search_result(update, context, [user_data], f"User ID: {search_id}")
                else:
                    await update.message.reply_text(
                        f"❗ *No user found with ID: {search_id}*\n\n"
            ...
            users = search_users_by_name(search_input)
            
            if users:
                await display_user_search_result(update, context, users, f"Name/Username: {search_input}")
            else:
                await update.message.reply_text(
                    f"❗ *No users found matching: '{escape_markdown_text(search_input)}'*\n\n"
        ...
        await update.message.reply_text(
            f"❗ *Search Error*\n\n"
            f"An error occurred while searching: {escape_markdown_text(str(e))}",
            parse_mode="MarkdownV2"
        )
    
    # Clear the search state
    context.user_data.pop('state', None)
    context.user_data.pop('admin_search_type', None)

async def display_user_search_result(update: Update, context: ContextTypes.DEFAULT_TYPE, users_data: list, search_query: str):
    """Display user search results with management options"""
    if not users_data:
        await update.message.reply_text(
    ...
    # Send header
    header_text = f"🔍 *User Search Results*\n\n*Search:* {escape_markdown_text(search_query)}\n*Found:* {len(users_data)} user(s)\n\n"
    await update.message.reply_text(header_text, parse_mode="MarkdownV2")
    
    import asyncio
    
    # Display each user with management buttons
    for user_data in users_data[:10]:  # Limit to 10 results
        user_id, username, first_name, last_name, join_date, questions_asked, comments_posted, blocked = user_data
        
        name = f"{first_name or ''} {last_name or ''}".strip() or "No name"
        status = "🚫 Blocked" if blocked else "✅ Active"
        
        user_text = f"*User Information*\n\n"
        user_text += f"*Name:* {escape_markdown_text(name)}\n"
        user_text += f"*ID:* `{user_id}`\n"
        user_text += f"*Username:* {f"@{escape_markdown_text(username)}" if username else "None"}\n"
        user_text += f"*Status:* {status}\n"
        user_text += f"*Joined:* {escape_markdown_text(join_date[:10]) if join_date else "Unknown"}\n"
        user_text += f"*Activity:* {questions_asked} posts, {comments_posted} comments\n"
        
        keyboard = [
            [
                InlineKeyboardButton(
                    "✅ Unblock" if blocked else "⛔ Block",
                    callback_data=f"admin_{'unblock' if blocked else 'block'}_{user_id}"
                ),
                InlineKeyboardButton(
                    "👤 Full Info",
...
"""
    ...
    # User search method callbacks
    if data == "admin_search_by_id":
        await admin_search_by_id(update, context)
        return
        
    if data == "admin_search_by_name":
        await admin_search_by_name(update, context)
        return
        
    if data == "admin_search_recent":
        await admin_search_recent_users(update, context)
        return
        
    if data == "admin_search_active":
        await admin_search_active_users(update, context)
        return
...
# User search database functions
def search_user_by_id(user_id_to_search):
...
def search_users_by_name_username(search_term, limit=10):
    """Search for users by name or username with partial matching"""
    db_conn = get_db_connection()
    with db_conn.get_connection() as conn:
        cursor = conn.cursor()
        placeholder = db_conn.get_placeholder()
        ...
        like_operator = "ILIKE" if db_conn.use_postgresql else "LIKE"
        
        cursor.execute(f'''
            SELECT user_id, username, first_name, last_name, join_date, 
                   questions_asked, comments_posted, blocked
            FROM users 
            WHERE (first_name {like_operator} {placeholder} OR 
                   last_name {like_operator} {placeholder} OR 
                   username {like_operator} {placeholder} OR 
                   CONCAT(first_name, ' ', last_name) {like_operator} {placeholder})
            ORDER BY (questions_asked + comments_posted) DESC
            LIMIT {placeholder}
...
async def display_user_search_results(update: Update, context: ContextTypes.DEFAULT_TYPE, users, search_term=""):
    """Display search results with user details and management options"""
    query = update.callback_query
    
    if not users:
        await query.edit_message_text(
            f"🔍 *User Search Results*\n\n❌ No users found{f' for "{escape_markdown_text(search_term)}"' if search_term else ''}\\.",
            reply_markup=InlineKeyboardMarkup([[
    ...
    # Build results text
    results_text = f"🔍 *User Search Results*\n\n"
    if search_term:
        results_text += f"📝 Search: \"{escape_markdown_text(search_term)}\"\n"
    results_text += f"📊 Found: {len(users)} user{'s' if len(users) != 1 else ''}\n\n"
    
    keyboard = []
    
    for i, user_data in enumerate(users[:15], 1):  # Limit to 15 results for display
        uid, username, first_name, last_name, join_date, questions_asked, comments_posted, blocked = user_data
        ...
        display_name = " ".join(name_parts) if name_parts else (username if username else "Anonymous")
        
        # Format join date
        try:
            if join_date:
                if isinstance(join_date, str):
                    join_date_str = join_date[:10]  # Take first 10 chars (YYYY-MM-DD)
                else:
        ...
        results_text += f"{i}\\. {status_emoji} *{escape_markdown_text(display_name)}*\n"
        results_text += f"   ID: `{uid}`"
        if username:
            results_text += f" \\| @{escape_markdown_text(username)}"
        results_text += f"\n   Activity: {questions_asked} posts, {comments_posted} comments\n"
        results_text += f"   Joined: {escape_markdown_text(join_date_str)}\n\n"
        
        # Add management buttons (for first 10 users to avoid button limit)
        if i <= 10:
            user_buttons = []
            user_buttons.append(InlineKeyboardButton(f"👤 {display_name[:8]}...", callback_data=f"admin_user_info_{uid}"))
            
            if blocked:
                user_buttons.append(InlineKeyboardButton("✅ Unblock", callback_data=f"admin_unblock_{uid}"))
            else:
                user_buttons.append(InlineKeyboardButton("⛔ Block", callback_data=f"admin_block_{uid}"))
...
async def admin_search_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show user search interface with multiple search options"""
    query = update.callback_query
    await query.answer()
    ...
    search_text = f"""🔍 *User Search*

*Search Options:*
• 🆔 Search by Telegram ID
• 👤 Search by name/username
• 📅 View recent users
• 🎯 Quick search most active

Choose a search method:"""
    
    keyboard = [
        [
            InlineKeyboardButton("🆔 Search by ID", callback_data="admin_search_by_id"),
            InlineKeyboardButton("👤 Search by Name", callback_data="admin_search_by_name")
        ],
        [
            InlineKeyboardButton("📅 Recent Users", callback_data="admin_search_recent"),
            InlineKeyboardButton("🎯 Most Active", callback_data="admin_search_active")
        ],
        [
            InlineKeyboardButton("🔙 Back to User Management", callback_data="admin_users")
        ]
...
*Search Methods:*
• Search by ID - Find user by exact user ID
• Search by Name - Find users by username/name
• Recent Users - Show recently joined users
• Active Users - Show users with recent activity
...
"""
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton("🕒 Recent Users", callback_data="admin_search_recent"),
            InlineKeyboardButton("⚡ Active Users", callback_data="admin_search_active")
        ],
        [
            InlineKeyboardButton("🔙 Back to User Management", callback_data="admin_users")
        ]
...
async def admin_search_by_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['state'] = 'admin_search_input'
    
    search_text = """
🆔 *Search by User ID*

Enter the exact user ID you want to search for:

Example: `123456789`

Type the user ID below or use the Cancel button to return to search menu\\.
"""
...
async def admin_search_by_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
Enter a name or username to search for\\. The search will look through:
• Usernames
• First names
• Last names

Example: `john`, `@username`, `smith`

Type the search term below \\(minimum 2 characters\\) or use the Cancel button to return to search menu\\.
"""
...
async def admin_search_recent_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show recent users"""
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        header_text = f"🕒 *Recent Users \\({len(recent_users)}\\)*\n\n📅 Recently joined users"
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=header_text,
            parse_mode="MarkdownV2"
        ...
        # Display users
        await display_user_list(update, context, recent_users, "recent")
        
    except Exception as e:
        logger.error(f"Error in admin_search_recent_users: {e}")
        await query.edit_message_text(
            "❗ *Error*\n\nFailed to load recent users\\. Please try again\\.",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔙 Back to Search", callback_data="admin_search_user")
            ]]),
            parse_mode="MarkdownV2"
...
async def admin_search_active_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show active users"""
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        header_text = f"⚡ *Active Users \\({len(active_users)}\\)*\n\n🔥 Users with recent activity"
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=header_text,
            parse_mode="MarkdownV2"
        ...
        # Display users
        await display_user_list(update, context, active_users, "active")
        
    except Exception as e:
        logger.error(f"Error in admin_search_active_users: {e}")
        await query.edit_message_text(
...
async def display_user_list(update: Update, context: ContextTypes.DEFAULT_TYPE, users, list_type):
    """Display a list of users with management buttons"""
    import asyncio
    
    # Display each user (limit to 10 results)
    for i, user_data in enumerate(users[:10], 1):
        ...
        if last_name:
            name_parts.append(last_name)
        display_name = " ".join(name_parts) or username or "Anonymous"
...
*Username:* {f"@{escape_markdown_text(username)}" if username else "None"}
*Status:* {status}
*Activity:* {total_activity} total actions
• Posts: {questions_asked}
• Comments: {comments_posted}
*Joined:* {escape_markdown_text(join_text)}"""
        
        # Create action buttons
        keyboard = []
        if blocked:
            keyboard.append([InlineKeyboardButton("✅ Unblock User", callback_data=f"admin_unblock_{uid}")])
        else:
            keyboard.append([InlineKeyboardButton("⛔ Block User", callback_data=f"admin_block_{uid}")])
        
        keyboard.append([InlineKeyboardButton("👤 Full Info", callback_data=f"admin_user_info_{uid}")])
...
async def handle_admin_search_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle admin search input for user search"""
    user_id = update.message.from_user.id
    text = update.message.text
    ...
    # Get the search type from context
    search_type = context.user_data.get('search_type')
    if not search_type:
        await update.message.reply_text("❗ Error: Search type not found.")
        await show_menu(update, context)
        return
    ...
    try:
        ...
        if search_type == 'by_id':
            ...
            try:
                ...
                results = search_user_by_id(search_user_id)
            except ValueError:
                await update.message.reply_text("❗ Invalid user ID. Please enter a valid number.")
                return
        elif search_type == 'by_name':
            # Search by name (username, first name, last name)
            search_term = text.strip()
            if len(search_term) < 2:
                await update.message.reply_text("❗ Search term must be at least 2 characters long.")
                return
            results = search_users_by_name(search_term)
        
        # Display results
        await display_user_search_results(update, context, results, search_type, text)
        
    except Exception as e:
        logger.error(f"Error in admin search input: {e}")
...
async def display_user_search_results(update: Update, context: ContextTypes.DEFAULT_TYPE, results, search_type, search_term):
    """Display user search results with management buttons"""
    if not results:
        no_results_text = f"🔍 *No Users Found*\n\nNo users found for search: {escape_markdown_text(search_term)}"
    ...
    search_type_text = "ID" if search_type == "by_id" else "name"
    header_text = f"🔍 *Search Results*\n\nFound {len(results)} user(s) for {search_type_text}: {escape_markdown_text(search_term)}"
...
• Posts: {questions_asked}
• Comments: {comments_posted}
*Joined:* {escape_markdown_text(join_date[:10]) if join_date else "Unknown"}"""
